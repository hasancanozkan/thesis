    ...
    for index in range (0,4,1):
            ...
            for i in range(len(param_bl[0])):
                for j in range(len(param_bl[1])):
                    ...
                    for k in range(len(param_x)):
                        ...
                        img_fr = img_as_ubyte(frangi(img_filtered,sigma,0.5,beta1=0.5,beta2= 0.125,  black_ridges=True))
                        ...
                        bl_class_result = bl_class_result +str(param_bl[0][i])+'_'+str(param_bl[1][j])+"_sigma_"+str(param_x[k])+"\n" + str(perform_result)+"\n"
                        cv2.imwrite(str(im)+'_Frangi_bl_'+str(param_bl[0][i])+'_'+str(param_bl[1][j])+"_sigma_"+str(param_x[k])+'.tif', img_fr_roi)
                        #save the data
            ...
            for i in range(len(param_ad[0])):
                for j in range(len(param_ad[1])):
                    ...
                    for k in range(len(param_x)):
                        ...
                        # performance measure
                        perform_result = perf_measure(labeled_crack, img_fr_roi, img_roi) 
                        ad_class_result = ad_class_result +str(param_ad[0][i])+'_'+str(param_ad[1][j])+"_sigma_"+str(param_x[k])+"\n" + str(perform_result)+"\n"
                        cv2.imwrite(str(im)+'_Frangi_ad_'+str(param_ad[0][i])+'_'+str(param_ad[1][j])+"_sigma_"+str(param_x[k])+'.tif', img_fr_roi)
                        #save the data
            ...
            for i in range(len(param_gf[0])):
                for j in range(len(param_gf[1])):
                    ...
                    for k in range(len(param_x)):
                        ...
                        # performance measure
                        perform_result = perf_measure(labeled_crack, img_fr_roi, img_roi) 
                        gf_class_result = gf_class_result +str(param_gf[0][i])+'_'+str(param_gf[1][j])+"_sigma_"+str(param_x[k])+"\n" + str(perform_result)+"\n"
                        cv2.imwrite(str(im)+'_Frangi_gf_'+str(param_gf[0][i])+'_'+str(param_gf[1][j])+"_sigma_"+str(param_x[k])+'.tif', img_fr_roi)
                        #save the data
        ...
        if (index == 3):
            ...
            for k in range(len(param_x)):
                    ...
                    cv2.imwrite(str(im)+"_sigma_"+str(param_x[k])+'_Frangi_waveApprox.tif', img_fr_roi)
                    wave_class_result = wave_class_result +"_sigma_"+str(param_x[k])+"\n" + str(perform_result)+"\n"

                    #save the data
                    with open(str(im)+'_Frangi_waveApprox_classification_results.txt','w') as output:
