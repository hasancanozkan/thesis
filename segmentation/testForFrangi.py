...
from ROI_function import createROI
...
for im in range(len(image_list)):    
    ...
    #sigma x-y
    param_x = [1.0,] # this is not important for only frangi it is just for loop
    param_y = [1.0]#changed
    #setting constants
    param_beta1= [0.5]
    param_beta2 = [0.125]
    ...
    img_filtered = guidedFilter(img_Eq, img_Eq, 2, 0.2)
    img_fr1 = img_as_ubyte(frangi(img_filtered, (1.0,5),0.5 ,beta1=0.5,beta2= 0.125,  black_ridges=True))
    img_fr2 = img_as_ubyte(frangi(img_filtered, (1.0,1.1),0.5 ,beta1=0.5,beta2= 0.125,  black_ridges=True))
    _, img_thresh = cv2.threshold(img_fr1,0,255,cv2.THRESH_BINARY)
    _, img_thresh2 = cv2.threshold(img_fr2,0,255,cv2.THRESH_BINARY)
    cv2.imwrite('deneme1.tif',img_thresh)
    cv2.imwrite('deneme2.tif',img_thresh2)
...
"""2 
    ...
    for index in range (0,4,1):
        ...
        if(index == 2):
                        ...
                        for z in range(len(param_beta2)):
                            for k in range(len(param_x)):
                                for l in range(len(param_y)):
                                        v.append(img_as_ubyte(frangi(img_filtered, (1.0,2.5),0.5 ,beta1=param_beta1[t],beta2= param_beta2[z],  black_ridges=True))) 
            ...
            cv2.imwrite('frangi'+str(im)+'gf_'+str(param_gf[0][i])+'_'+str(param_gf[1][j])+ '_beta1_' + str(param_beta1[t]) + '_beta2_' + str(param_beta2[z])+'.tif', img_thresh)
            cv2.imwrite('frangi'+str(im)+'ROI_gf_'+str(param_gf[0][i])+'_'+str(param_gf[1][j])+ '_beta1_' + str(param_beta1[t]) + '_beta2_' + str(param_beta2[z])+'.tif', img_fr_roi)
