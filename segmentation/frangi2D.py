...
labeled_crack = cv2.dilate(labeled_crack,kernel,iterations =2)
...
kernel = np.ones((2,2),np.uint8)
img_fr = cv2.erode(img_fr,kernel,iterations =2)
...
if MORPH:
    ...
    img_morph1 = cv2.morphologyEx(img_thresh, cv2.MORPH_OPEN, kernel)
    
    kernel = np.ones((1,10),np.uint8)
    img_morph2 = cv2.morphologyEx(img_thresh, cv2.MORPH_OPEN, kernel)
